...
class PDFBaseDoc extends Buffer {
    ...
    protected static function build_xref_1_5($offsets) {
        ...
        for ($i = 0; $i < count($k); $i++) {
            ...
            if (is_array($c_offset)) {
                $result .= pack('C', 2);
                $result .= pack('N', $c_offset[0]);
                $result .= pack('C', $c_offset[1]);
            } else {
                if ($c_offset === null) {
                    $result .= pack('C', 0);
     ...
     */
    protected static function get_xref_1_5(&$_buffer, $xref_pos) {
    ...
    protected static function get_xref(&$_buffer, $xref_pos) {
        ...
        if ($trailer_pos === false) {
            [ $xref_table, $trailer_obj, $min_pdf_version ] = self::get_xref_1_5($_buffer, $xref_pos);
        ...
        if (isset($trailer_obj['Prev'])) {
            ...
            [ $prev_table, $prev_trailer, $prev_min_pdf_version ] = self::get_xref($_buffer, $xref_prev_pos);
    ...
    protected static function acquire_structure(&$_buffer) {
        ...
        [ $xref_table, $trailer_object, $min_pdf_version ] = self::get_xref($_buffer, $xref_pos);
     ...
     */
    protected static function find_object_at_pos(&$_buffer, $oid, $object_offset, $xref_table) {
     ...
     */
    protected static function find_object_in_objstm(&$_buffer, $xref_table, $objstm_oid, $objpos, $oid) {
